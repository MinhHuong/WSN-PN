...
namespace PAT.Module.PN.Model
{
    public class PNTransition : StateItem
    {
        ...
        {
            get { return _probabilityPathCongestion; }
            set { _probabilityPathCongestion = (value > 1) ? value / 100d : value; }
        }
        #endregion

        public PNTransition(string name, string id, int color, double prob = 0.0)
            : base(false, name)
        {
            ...
            // Prob only applies for Transitions of abstracted Channel
            _probabilityPathCongestion = prob;
        }
